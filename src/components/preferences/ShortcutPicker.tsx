...
/** Characters to their shifted (uppercase) equivalents for display. */
...
function formatShortcutForDisplay(shortcut: string): string {
  ...
  // When Shift is a modifier, show the shifted character (e.g. , → <, . → >)
  if (hasShift) {
    formatted = formatted.replace(/[^⌘⌃⇧⌥↵⌫⌦↑↓←→⇥\s]$/, c => SHIFT_MAP[c] ?? c)
  }
